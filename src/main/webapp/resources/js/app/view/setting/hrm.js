define([
	...
	return B.View.extend({
		events: {
			...
			'click .ac_removeHrmItem': 'removeHrmItem',	// 删除人员
			'change .ac_uploadExcel': 'uploadExcel',	// 导入Excel
			'click .ac_exportHrm': 'exportHrm',			// 导出人员
			'click .ac_delAll': 'delAllHrmItem'			// 全部删除
		},
		...
		renderList: function(options) {
			...
			$.ajax({
				type: 'GET',
				dataType: 'JSON',
				url: requestConfig.setting.hrm + '?' + _data
			}).done(function (r) {
				if(r.resultType === 'true') {
					var _hrmTPL = $('#workloadHrm').html(),
						_operationTPL = $('#workloadOperation').html();
					var _hrmList = _.template(_hrmTPL)({'list': r.result, 'resultRight': r.resultRight}),
						_operationList = _.template(_operationTPL)({'resultRight': r.resultRight, 'hrmTypeList': r.hrmTypeList});
		...
		editHrmItem: function(e) {
			...
			var _oprt = $target.data('oprt'),
				_projectFormTPL = $('#hrmForm').html(),
				_spriteTitle = _oprt==='add' ? '新建人员' : '修改人员',
				_spriteOption = {
					oprt: _oprt,
					objType: 1
				};
			...
			if(_oprt === 'edit') {
				_spriteOption = {
					...
					id: $tr.data('id'),
					name: $tr.data('name'),
					loginId: $tr.data('badge'),
					email: $tr.data('email'),
					managerLogin: $tr.data('managerlogin'),
					managerName: $tr.data('managername'),
					deptId: $tr.find('td.dept-item').data('id'),
					deptName: $tr.find('td.dept-item').data('name'),
					managerHrmId: $tr.find('td.manager-item').data('id'),
					managerHrmName: $tr.find('td.manager-item').data('name'),
					objType: $tr.data('system'),
					hrmTypeId: $tr.find('td.hrm-item').data('id'),
					hrmTypeName: $tr.find('td.hrm-item').data('name'),
					objStatus: $tr.data('status')
				}
			}

			// 人员类型列表
			_spriteOption.hrmTypeList = me.hrmTypeList;
			...
			var $projectSprite = interactiveEvent.sprite({
				...
				onconfirm: function(obj, target) {
					...
					$.ajax({
						type: 'POST',
						dataType: 'JSON',
						url: requestConfig.setting.hrmOprt,
		...
		removeHrmItem: function(e) {
			...
			interactiveEvent.removeComfirm($target, '确定删除该人员？', function() {
				$.ajax({
					type: 'GET',
					dataType: 'JSON',
					url: requestConfig.setting.hrmOprt + '?oprt=del&id=' + _id
				}).done(function (r) {
					if(r.resultType === 'true') {
						$.oaTip(r.resultMsg, 'success', 1000, function() {
							me.removeRefresh();
		...
		// 全部删除人员
		delAllHrmItem: function(e) {
			var $target = $(e.currentTarget);
			interactiveEvent.removeComfirm($target, '确定删除全部人员？', function() {
				$.ajax({
					type: 'GET',
					dataType: 'JSON',
					url: requestConfig.setting.hrmOprt + '?oprt=delAll'
				}).done(function (r) {
					if(r.resultType === 'true') {
						$.oaTip(r.resultMsg, 'success', 1000, function() {
							$tr.remove();
		...
		exportHrm: function() {
			...
			$.ajax({
				type: 'GET',
				dataType: 'JSON',
				url: requestConfig.setting.hrmExp + '?' + _data
			}).done(function (r) {
				if(r.resultType === 'true') {
