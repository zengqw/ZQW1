	...
	$.cxCalendar = function() {
		...
		calendar.build = function(isFirst) {
			...
			if (isFirst === true) {
				...
				if (self.settings.isMonPicker){
					self.dom.footer = $('<div></div>', {'class': 'footer-control'}).appendTo(self.dom.paneHd);
					self.dom.confirm = $('<i></i>', {'class': 'btn-confirm'}).appendTo(self.dom.footer);
				}
		...
		calendar.bindEvents = function() {
			...
			// 关闭面板
			self.dom.el.on("closeBox", function(){
				self.hide();
			});
			// 关闭面板
			...
			if (self.settings.isMonPicker){
				self.dom.confirm.on('click', function(){
					self.setDate(self.dom.yearSet.val(), self.dom.monthSet.val(), 1);  //默认为选择月1号
				});
			}
		...
		calendar.show = function() {
			...
			//如果是年月选择器，则隐藏日选择面板,且高度只为年月选择器的高度
			if (self.settings.isMonPicker){
				self.dom.paneBd.hide();
				_paneHeight = 42;
			}
