...
(function(win, undef) {
	...
	var AESEC = {
		//ÓÃ·¨£ºAESEC.strEnc(sendData,key,userId+jsEssionId)
		strEnc:function(word,k1,k2){
			k1 = getTheLenStr(k1,16);
			k2 = getTheLenStr(k2,16);
			var key = CryptoJS.enc.Utf8.parse(k1);   
			var iv  = CryptoJS.enc.Utf8.parse(k2);   
			var srcs = CryptoJS.enc.Utf8.parse(word);  
			var encrypted = CryptoJS.AES.encrypt(srcs, key, { iv: iv,mode:CryptoJS.mode.CBC});  
			//return encodeURIComponent(encrypted.toString());  
			return encrypted.toString();  
		}, 
		strDec:function(word,k1,k2){  
			k1 = getTheLenStr(k1,16);
			k2 = getTheLenStr(k2,16);
		    var key = CryptoJS.enc.Utf8.parse(k1);   
			var iv  = CryptoJS.enc.Utf8.parse(k2);  
			var decrypted = CryptoJS.AES.decrypt(word,key,{iv:iv,padding:CryptoJS.pad.ZeroPadding}).toString(CryptoJS.enc.Utf8);
			return decrypted;
		}  
	};
	function getTheLenStr(str,len){
		var helperAry = [];
		str = str.length >= len?str.substring(0,len):(helperAry.length = (len-str.length+1),str+helperAry.join("0"));
		return str;
	}
	win.AESEC = AESEC;
})(window);